...
/* ── Safe hex printer (no leading "0x" — avoids serial_write_hex bug) ─── */
static void kprint_hex(uint64_t v) {
...
static void parse_multiboot(uint32_t mb_info_phys) {
    /* mb_info_phys is a 32-bit physical address from boot64.s.
     * Under the identity map boot64.s sets up, PA == VA for low memory. */
    if (!mb_info_phys) { kprint("[MEM] No multiboot info pointer\n"); return; }

    /* The Multiboot2 info structure starts with: u32 total_size, u32 reserved.
     * Tags begin at offset +8. */
    uint8_t *ptr = (uint8_t *)(uintptr_t)mb_info_phys + 8;
    uint32_t total_size = *(uint32_t *)(uintptr_t)mb_info_phys;

    /* Sanity check: total_size must be >8 and <4 MiB (a real MB2 struct
     * is never larger than a few KB; anything bigger means a bad pointer). */
    if (total_size < 8 || total_size > 4 * 1024 * 1024) {
        kprint("[MEM] Multiboot info size looks invalid — skipping mmap\n");
        return;
    }
    ...
    for (uint32_t iter = 0; iter < MB2_MAX_ITER; iter++) {
        ...
        if (tag->size < 8) break;  /* malformed */

        if (tag->type == MB2_TAG_MMAP) {
            mb2_tag_mmap_t *mt = (mb2_tag_mmap_t *)ptr;
            uint32_t es = mt->entry_size;

            /* entry_size must be at least 24 bytes (the struct size) and
             * a multiple of 8.  If it's not, bail — bad data. */
            if (es < 24 || es > 256 || (es & 7)) {
                kprint("[MEM] Bad mmap entry_size — skipping\n");
            } else {
                uint8_t *ep  = ptr + sizeof(mb2_tag_mmap_t);
                uint8_t *end = ptr + tag->size;
                kprint("[MEM] Memory map:\n");
                while (ep + es <= end) {
                    mb2_mmap_entry_t *e = (mb2_mmap_entry_t *)ep;
                    if (e->type == 1) {
                        kprint("[MEM]   avail  base=");
                        kprint_hex(e->base_addr);
                        kprint("  len=");
                        kprint_hex(e->length);
                        kprint("\n");
                    }
        ...
        if (next == 0) break;  /* prevent infinite loop */
        ptr += next;
    }
}
