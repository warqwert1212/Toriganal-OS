...
//
// FIX: renamed from loader_enhaced.c (typo), and normalized all includes
// to the flat kernel/ + include/ layout (-Iinclude -Ikernel) instead of
// fragile "../include/x.h" relative paths.
// ==============================================================================
...
int loader_load_exe(const char *filename, pid_t pid) {
    ...
    if (size < 0x40) { serial_puts("[PE] too small for DOS header\n"); kfree(buf); return -1; }
