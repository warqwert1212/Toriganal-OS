...
#include "vmm.h"
#include "heap.h"
#include "trpfs.h" // Added to define the filesystem functions and structures

// Forward declaration of the global ramdisk structure
...
void ahci_init(uint64_t hba_base_phys) {
    ...
    // Initialize and mount the ramdisk inside the actual function body
    print_serial("[SATA] Initializing RAM disk...\n");
    trpfs_ramdisk_init(&g_disk, 32ULL * 1024 * 1024);
    trpfs_mount(&g_disk);
}
