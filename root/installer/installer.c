...
#define INSTALLER_DISK_BYTES   (32ULL * 1024ULL * 1024ULL)  /* 32 MiB TRPFS volume */
...
static int provision_disk(installer_errors_t *errs) {
    ...
    serial_write_dec_compat: ;
    return 0;
}

/* serial_write_dec may not exist on all builds of serial.h's small API in
 * every config, so keep diagnostics to serial_puts() only. */
