...
#include "fs.h"
#include "loader.h"
#include "keybord.h"      // keyboard driver (correct header name)
...
static void kernel_install_toriginal_os(void) {
    ...
    if (fs_mkdir("/toriginal_os", FILE_PERM_OWNER_R | FILE_PERM_OWNER_W | FILE_PERM_OWNER_X) == 0)
        ...
        kernel_install_log("/toriginal_os exists or could not be created\n");
    ...
    /* Prompt user for simple configuration */
    io_put_string("Enter username: ");
    for (int i = 0; ; ) {
        char c = keyboard_getc();
        if (c == '\r' || c == '\n') break;
        if ((c == '\b' || c == 127) && i > 0) { i--; io_put_char('\b'); io_put_char(' '); io_put_char('\b'); }
        else if (i + 1 < (int)sizeof(username)) { username[i++] = c; io_put_char(c); }
    }
    io_put_string("\n");
    ...
    for (int i = 0; ; ) {
        ...
        if (c == '\r' || c == '\n') break;
        if ((c == '\b' || c == 127) && i > 0) { i--; io_put_char('\b'); io_put_char(' '); io_put_char('\b'); }
        else if (i + 1 < (int)sizeof(password)) { password[i++] = c; io_put_char('*'); }
    }
    io_put_string("\n");
    ...
    for (int i = 0; ; ) {
        ...
        if (c == '\r' || c == '\n') break;
        if ((c == '\b' || c == 127) && i > 0) { i--; io_put_char('\b'); io_put_char(' '); io_put_char('\b'); }
        else if (i + 1 < (int)sizeof(timezone)) { timezone[i++] = c; io_put_char(c); }
    }
    io_put_string("\n");

    io_put_string("Accent color (blue/green/red): ");
    for (int i = 0; ; ) {
        char c = keyboard_getc();
        if (c == '\r' || c == '\n') break;
        if ((c == '\b' || c == 127) && i > 0) { i--; io_put_char('\b'); io_put_char(' '); io_put_char('\b'); }
        else if (i + 1 < (int)sizeof(accent)) { accent[i++] = c; io_put_char(c); }
    }
    io_put_string("\n\n");

    /* Persist configuration to installed tree */
    fd_t fd = fs_open("/toriginal_os/config.ini", O_CREAT | O_WRONLY | O_TRUNC, FILE_PERM_OWNER_R | FILE_PERM_OWNER_W);
    if (fd >= 0) {
        fs_write(fd, "username=", 9);
        fs_write(fd, username, strlen(username));
        fs_write(fd, "\n", 1);

        fs_write(fd, "password=", 9);
        fs_write(fd, password, strlen(password));
        fs_write(fd, "\n", 1);

        fs_write(fd, "timezone=", 9);
        fs_write(fd, timezone, strlen(timezone));
        fs_write(fd, "\n", 1);

        fs_write(fd, "accent=", 7);
        fs_write(fd, accent, strlen(accent));
        fs_write(fd, "\n", 1);

        fs_close(fd);
        kernel_install_log("Wrote /toriginal_os/config.ini\n");
    } else {
        kernel_install_log("Failed to write /toriginal_os/config.ini\n");
    }
    ...
    kernel_install_log("Interactive installer finished.\n");
}
