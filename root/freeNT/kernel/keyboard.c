 ...
 *
 * REVISION NOTES:
 *   - keyboard_init() unmasks IRQ1 on the master PIC (needed if a PIC
 *     init/remap routine masked everything at boot).
 *   - The 8042 config byte is now set with a KNOWN VALUE (0x01: IRQ1
 *     enabled, IRQ12/mouse-clock left alone by mouse_init() running after
 *     us, translation OFF so raw set-1 scancodes reach us unmodified)
 *     instead of read-modify-write. A read-modify-write trusts
 *     kbc_recv_data()'s bounded timeout to have returned a REAL config
 *     byte; if that read ever times out under a picky virtual 8042, we'd
 *     OR our bit into garbage and write the garbage straight back,
 *     potentially corrupting clock/translation bits and killing the
 *     keyboard entirely. Writing a known-good byte sidesteps that.
 * ============================================================================= */
...
/* Config byte we write:
 *   bit0 (0x01) = port-1 (keyboard) IRQ enable — the one we actually need
 *   bit1 (0x02) = port-2 (mouse) IRQ enable — mouse.c (mouse_init(),
 *                 called right after us in kernel.c) never sets this bit
 *                 itself, it only enables the aux port's clock via 0xA8.
 *                 If we clear bit1 here, IRQ12 silently never fires and
 *                 the mouse looks "dead" the same way the keyboard did.
 *   bit2 (0x04) = "system flag" (POST passed) — some 8042 implementations
 *                 treat this getting cleared as an error-ish state; we
 *                 preserve it rather than risk it.
 *   All other bits 0: no scancode translation (bit6, we want raw set-1
 *   ourselves), both port clocks enabled (bits 4/5 = 0 means enabled). */
#define I8042_CFG_KBD_AND_MOUSE_IRQ (0x01 | 0x02 | 0x04)
...
void keyboard_init(void)
{
    ...
    /* Write a KNOWN-GOOD config byte (IRQ1 enabled, nothing else) rather
     * than reading the current byte and OR-ing a bit in. See file header
     * for why read-modify-write is riskier here. */
    kbc_send_cmd(I8042_CMD_WRITE_CFG);
    kbc_send_data(I8042_CFG_KBD_AND_MOUSE_IRQ);
