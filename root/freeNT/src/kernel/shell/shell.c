...
static void kernel_run_executable(const char *path) {
    ...
    /* Create a process execution frame context with priority 1 */
    process_t *new_proc = process_create(path, 1);
    if (!new_proc) {
        io_put_string("Execution failed: Could not create process context.\n");
        serial_puts("Failed to create process\n");
        return;
    }

    /* Extract the pid field safely from the process structure pointer */
    pid_t new_pid = new_proc->pid;

    /* Hand off the binary file to the generic format loader */
    if (loader_load_executable(path, new_pid) == 0) {
        process_start(new_pid);
        serial_puts("Process executed successfully\n");
    } else {
        io_put_string("Execution failed: Format handler error.\n");
        serial_puts("Exec failed or processed via internal VM\n");
    }
}
...
static void kernel_handle_command(char *command, int is_os_mode) {
    ...
    /* Intercept implicit program calls by checking extensions (.exe, .trp, .txt) */
    if (cmd_len > 4 && 
