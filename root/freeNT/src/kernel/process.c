...
/* External declaration linking directly to your native binary loader module */
extern int load_and_execute_user_elf(uint8_t* raw_elf_data);
...
int process_exec(pid_t pid, const char *filename, const char **argv) {
    ...
    /* Fixed: Call our real native ELF layout parser loader module */
    int r = load_and_execute_user_elf((uint8_t*)filename);
    if (r != 0) {
